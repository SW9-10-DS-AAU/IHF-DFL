...
def plot_from_parsed(
    ...
    try:
        for p in participants_list:
            ...
            if getattr(p, "states", None):
                ...
                att = getattr(last_state, "futureAttitude", None)
    ...
    if use_step_grs:
        ...
        for p_idx, series in enumerate(per_user_grs):
            ...
            if not xs:
                continue
            axs[2].plot(xs, ys, linewidth=2.0, color=colors[p_idx % len(colors)], alpha=0.9, marker=markers[attitudeArr[p_idx]], markersize=5)
            legend_elements[attitudeArr[p_idx]] = Line2D([0], [0], marker=markers[attitudeArr[p_idx]], color=colors[p_idx % len(colors)], linestyle="None", label=attitudeArr[p_idx])

    # --- panel (d): rewards & punishments ---
    axs[3].clear()
    if x_reward_step:
        axs[3].plot(x_reward_step, y_reward, label="reward", color=palette[0], linewidth=2.5)
        axs[3].plot(x_reward_step, y2_reward, label="reward + punishments", color=palette[1], linewidth=2.5, linestyle=(0, (4, 2)))
    ...
    info_text = (
        ...
        f"CSS={experiment_config.contribution_score_strategy}\n"
        f"outlier={experiment_config.use_outlier_detection}\n"
        f"noiseAmount={experiment_config.freerider_noise_scale}\n"
        f"noiseRound={experiment_config.freerider_start_round}\n"
        f"rounds={rounds_count}\n"
        f"users={users_count+malicious+freerider}\n"
        f"malicious={malicious}\n"
        f"freerider={freerider}"
    )
    ...
    legend_elements_arr = list(legend_elements.values())
    
    axs[2].legend(handles=legend_elements_arr, loc="upper left", bbox_to_anchor=(-0.2, -0.08), fontsize=9, frameon=True)
    ...
    axs[1].legend(twin_lines, [l.get_label() for l in twin_lines], loc="upper left", fontsize=9, bbox_to_anchor=(-0.2, -0.08))

    if x_reward_step:
        legend_handles = [
            Line2D([0], [0], color=palette[0], linewidth=2.5, label="reward"),
            Line2D(
                [0], [0],
                color=palette[1],
                linewidth=2.5,
                linestyle=(0, (4, 2)),
                label="reward + punishments"
            ),
        ]
        lgnd = axs[3].legend(handles=legend_handles ,fontsize=9, bbox_to_anchor=(-0.2, -0.08), loc="upper left")
